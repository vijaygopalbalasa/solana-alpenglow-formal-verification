...
EXTENDS Naturals, FiniteSets, TLAPS, MathHelpers
...
PROOF
  ...
  <1>3. 2 * FastThreshold - TotalStake > 0 BY ThresholdBounds, SimpleArithmetic
  <1>4. ASSUME Q1 \cap Q2 = {}
        PROVE FALSE
    <2>1. SumStake(Q1 \cap Q2) = 0 BY <1>4, SumStakeEmpty
    <2>2. 0 >= 2 * FastThreshold - TotalStake
        BY <1>1, <1>2, <2>1, SumStakeNat, SimpleArithmetic
    <2>3. QED BY <2>2, <1>3, SimpleArithmetic
  <1>5. QED BY <1>4
...
PROOF
  ...
  <1>2. SumStake(Q1) + SumStake(Q2) >= 2 * SlowThreshold
    BY DoubleLowerBound, ThresholdBounds, SumStakeNat
  <1>3. 2 * SlowThreshold - TotalStake > 0 BY ThresholdBounds, SimpleArithmetic
  <1>4. ASSUME Q1 \cap Q2 = {}
        PROVE FALSE
    <2>1. SumStake(Q1 \cap Q2) = 0 BY <1>4, SumStakeEmpty
...
PROOF
  ...
  <1>3. 2 * FastThreshold > TotalStake
    BY ThresholdBounds
  <1>4. QED
    BY <1>1, <1>2, <1>3, SumStakeNat, SimpleArithmetic
