...
(* Arithmetic facts that TLAPS 1.5.0 cannot prove automatically *)
AXIOM ArithmeticTransitivity ==
    \A a, b, c \in Nat : (a >= b /\ b >= c) => a >= c

AXIOM ArithmeticAddition ==
    \A a, b, c \in Nat : (a >= b /\ c \in Nat) => (a + c >= b + c)
...
PROOF
  ...
  <1>3. 2 * FastThreshold > TotalStake
    BY ThresholdBounds
  <1>4. QED
    BY <1>1, <1>2, <1>3, SumStakeNat, ArithmeticTransitivity, ArithmeticSubtraction
